...
public abstract class FieldSet extends AbstractComponent {

    @FindBy(className = "ui-fieldset-legend")
    private WebElement legend;

    @FindByParentPartialId("_collapsed")
    private WebElement collapsed;

    @FindBy(className = "ui-fieldset-content")
    private WebElement content;

    public void toggle() {
        if (PrimeGraphene.hasAjaxBehavior(root, "toggle")) {
            try {
                Graphene.guardAjax(legend).click();
            }
